...
class Helper
{
     ...
     * @return string
     */
    public static function generateInspectionFilePath($inpsection_id)
    {
        return storage_path('app/pw/inspections/' . $inpsection_id);
    }
    ...
    public static function getRedirectUrlAfterLogin($roleName)
    {
        $redirects = [
            ...
            'concessionaries' => route('dashboard.index')
        ];
        try {
    ...
    {
        $emails = ['cool.webpro17@gmail.com'];
        switch ($type) {
            case 'bkpa':
